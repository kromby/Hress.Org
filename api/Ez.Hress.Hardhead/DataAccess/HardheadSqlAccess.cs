...
public class HardheadSqlAccess : IHardheadDataAccess
{
    ...
    public async Task<IList<HardheadUserEntity>> GetUsersByYear(int yearId)
    {
        var sql = @"SELECT	usr.Id, usr.Username, usr.Inserted, uimg.ImageId, COUNT(usr.Id) attended
                        FROM rep_Event hh
                        JOIN    rep_User hhUsr ON hh.Id = hhUsr.EventId
        ...
        _log.LogInformation("[{Class}] Getting users for year: {yearId}", nameof(HardheadSqlAccess), yearId);
        _log.LogInformation("[{Class}] Executing SQL: '{SQL}'", nameof(HardheadSqlAccess), sql);

        var list = new List<HardheadUserEntity>();
        using (var connection = new SqlConnection(_connectionString))
        {
            await connection.OpenAsync();
            ...
            {
                var user = new HardheadUserEntity()
                {
                    ID = reader.GetInt32(reader.GetOrdinal("Id")),
                    Username = reader.GetString(reader.GetOrdinal("Username")),
                    Inserted = reader.GetDateTime(reader.GetOrdinal("Inserted")),
