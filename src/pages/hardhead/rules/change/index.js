...
const RuleChange = () => {
  ...
  const allowSaving = (category, text, newReasoning, rule) => {
    ...
    if (newReasoning === undefined || newReasoning.length <= 10) return false;

    if (selected === RuleChange.CREATE || selected === RuleChange.UPDATE) {
      if (text === undefined || text.length <= 16) return false;
    }

    if (selected === RuleChange.UPDATE || selected === RuleChange.DELETE) {
      if (rule === undefined || rule === "") return false;
    }
  ...
  const handleTextChange = (event) => {
    ...
    setButtonEnabled(
      allowSaving(selectedCategory, event.target.value, reasoning, selectedRule)
    );
  };
  const handleReasoningChange = (event) => {
    setReasoning(event.target.value);
    setButtonEnabled(
      allowSaving(selectedCategory, ruleText, event.target.value, selectedRule)
    );
  };
  const handleRuleChange = (event) => {
    setSelectedRule(event.target.value);
    setButtonEnabled(
      allowSaving(selectedCategory, ruleText, reasoning, event.target.value)
    );
  };
  ...
  const handleCategoryChange = (event) => {
    ...
    setButtonEnabled(
      allowSaving(event.target.value, ruleText, reasoning, selectedRule)
    );
  };
          ...
          <div className="row gtr-uniform" key="first">
            ...
            >
              <input
                id="update"
                type="radio"
                checked={selected === ChangeType.UPDATE}
                onChange={() => handleTypeChange(ChangeType.UPDATE)}
              />
              <label htmlFor="update">Reglubreyting</label>
            </div>
            <div
              className="col-4"
            ...
            >
              <input
                id="create"
                type="radio"
                checked={selected === ChangeType.CREATE}
                onChange={() => handleTypeChange(ChangeType.CREATE)}
              />
              <label htmlFor="create">Ný regla</label>
            </div>
            <div
              className="col-4"
            ...
            >
              <input
                id="delete"
                type="radio"
                checked={selected === ChangeType.DELETE}
                onChange={() => handleTypeChange(ChangeType.DELETE)}
              />
              <label htmlFor="delete">Fjarlægja reglu</label>
            </div>
          </div>,
          <hr key="Line2" />,
            ...
            {selected ? (
              <div className="row gtr-uniform">
                <div className="col-12">
                  ...
                  >
                    <option value="">
                      - Í hvaða flokki viltu breyta reglu? -
                    </option>
                ...
                (selected === ChangeType.UPDATE ||
                  selected === ChangeType.DELETE) ? (
                  <div className="col-12">
                    <select
                      id="rule"
                      name="rule"
                      onChange={(ev) => handleRuleChange(ev)}
                    >
                      <option value="">
                        {selected === ChangeType.UPDATE
                          ? "- Hvaða reglu viltu breyta? -"
                          : "Hvaða reglu viltu fjarlægja?"}
                      </option>
                      {rules.map((rule) => (
                        <option key={rule.id} value={rule.id}>
                ...
                selected === ChangeType.UPDATE ? (
                  <div className="col-12">
                    <textarea
                      ...
                      placeholder={
                        selected === ChangeType.UPDATE
                          ? "Hvernig viltu hafa regluna eftir breytingu?"
                          : "Hvernig viltu hafa nýju regluna?"
                      }
                    />
                  </div>
                ) : null}
