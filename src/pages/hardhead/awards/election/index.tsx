...
const Election = () => {
  ...
  const getElement = (ID: number, Name: string, Href: string) => {
    if (ID === 50) {
      ...
      /*Lög og reglur - breytingar*/ return (
        <Rules key={ID} ID={ID} Name={Name} onSubmit={handleSubmit} />
      );
    } else if (ID === 102) {
      /* Mynd fyrir uppgjörskvöldið */ return (
        <TwentyYearOldMovie
          key={ID}
          ID={ID}
          Name={Name}
      ...
      /*Vonbrigði*/ return (
        <Disappointment key={ID} ID={ID} Name={Name} onSubmit={handleSubmit} />
      );
    } else if (ID === 361) {
      return (
        <MovieOfTheYear
      ...
      /*Nýliði*/ return <Post key={ID} id={ID} title={Name} />;
    } else if (ID === 364) {
      return (
        <HardheadOfTheYear
          key={ID}
